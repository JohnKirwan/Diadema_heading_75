...
while i <= size(trial_limits, 1) ;
 ...
 [~, index] = min(abs(tranz_trackz{i}(:,2) - 0.75)) ; %get rho closest to .75
 destinations(i,1:2) = tranz_trackz{i}(index, 1:2) ; % get polar coords of this rho
...
while k <= num_trials 
    % modified from -(results_matrix(k,1)) to give destinatio instead of calculated heading 
    calc_points(k) = -(results_matrix(k,3)) + (pi/2) ;  % set to radian convention to be clockwise and zero at 0
