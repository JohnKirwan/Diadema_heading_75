...
while i <= size(trial_limits, 1) ;
 ...
 % [~, index] = min(abs(tranz_trackz{i}(:,2) - 0.25)) ; % ORIGIN
 circleValues(i,1:2) = tranz_trackz{i}(index, 1:2)  ; % ORIGIN
 
 [~, index] = min(abs(tranz_trackz{i}(:,2) - 0.5)) ; % GET CLOSEST PT TO 1/2 CIRCLE WIDTH
